...
import com.kitri.pos.stat.*;
import com.kitri.pos.Stock.StockBtn;
import com.kitri.pos.Stock.StockDao;
import com.kitri.pos.Stock.StockMonitor;
import com.kitri.pos.Stock.StockPopupChg;
import com.kitri.pos.Stock.StockPopupIn;
import com.kitri.pos.Stock.StockPopupSearch;
...
//		기능버튼패널 카드 각기능버튼 패널 객체생성후 추가
		...
		// 계정 이벤트 리스너 등록
		admin.getAuthority().addActionListener(this);
		admin.getAuthorityUp().addActionListener(this);
		...
		// 계정기능 마우스 클릭 이벤트
		admin.getTable().addMouseListener(ms);
...
//			회원 검색할 때 조건
		...
		if (ob == admin.getAuthority()) {
			// 콤보박스 안에서 선택한 값의 문자열을 얻어옴
			String str = admin.getAuthority().getSelectedItem().toString();
			if (str.equals("관리자")) {
				admin.auth = "T";
		...
		if (ob == admin.getAuthorityUp()) {
			String str = admin.getAuthorityUp().getSelectedItem().toString();
			if (str.equals("관리자")) {
				admin.auth = "T";
			} else if (str.equals("직원")) {
				admin.auth = "F";
		...
		// 인연이라고하죠
		if (obb.equals("유저등록")) {
			admin.card.show(admin.getpMonitor(), "pRegister");
			admin.tfClear();
		}
		...
		if (obb.equals("유저수정")) {
			admin.tfUClear();
			// 행의 번호를 뽑아옴.
			int numberRow = admin.getTable().getSelectedRow();

			if (admin.getTable().getSelectedRow() < 0) { // 테이블 말고 다른 곳을 클릭 했을 경우
				JOptionPane.showMessageDialog(this, "테이블을 클릭해주세요.");
			} else {
				String id = (String) admin.getTm().getValueAt(numberRow, 2);
				admin.card.show(admin.getpMonitor(), "ppRegister");

				// 유저수정 아이디 텍스트필드값을 아이디로 고정
			//dmin.upuserTF.setText(id);
			}
		}
		...
		if (obb.equals("유저삭제")) {
			...
			if (admin.getTable().getSelectedRow() < 0) {
				JOptionPane.showMessageDialog(this, "테이블을 클릭해주세요.");
				...
				if (x == JOptionPane.OK_OPTION) {
					admin.deleteUser();
					try {
						userDao.userSelectAll(admin.getTm());
		...
		if (obb.equals("확인")) {
			...
			UserDto re = admin.getViewData(); // 실제 넘어간 데이터 userDto에 저장.
			admin.setUserDao(new UserDao()); // userDao 객체 생성

			if (admin.isResult()) { // 데이터가 넘어 갔다면.
				try {
					admin.getUserDao().getMemberList(); // select 문 실행.
					admin.getUserDao().insertMember(re); // insert문 실행.
					admin.getUserDao().userSelectAll(admin.getTm());
				} catch (SQLException e1) {
					System.out.println("등록 실패");
					e1.printStackTrace();
				}
			} else {
				admin.setResult(false);
				return;
			}
		}
		...
		if (obb.equals("취소")) {
			...
			admin.card.show(admin.getpMonitor(), "pTable");
		}

		// 말그대로 로그아웃 로그인 화면으로 넘어감.
		if (obb.equals("로그아웃")) {
			this.setVisible(false);
			admin.setForcePos(new ForcePos());
			admin.getForcePos().setVisible(true);
		}
	...
	MouseAdapter ms = new MouseAdapter() {
		...
		public void mouseClicked(MouseEvent e) {
			...
			admin.setTable((JTable) e.getComponent());
			admin.setTm((DefaultTableModel) admin.getTable().getModel());
		}
	};
...
//		receipt.refundDetail.append("취소영수증/n" + "판매코드 : \t" );
	...
	private Vector<PosDto> productsHoldProcess() {
		...
		for (int i = 0; i < rowsNum; i++) {
			...
			row.setSellId((String) viewSalesInput.model.getValueAt(i, 0));
			...
			row.setSellCount(Integer.parseInt(String.valueOf(viewSalesInput.model.getValueAt(i, 4))));
	...
	private void productsHoldCancelProcess(Vector<PosDto> beforeList) {
		...
		for (int i = 0; i < bfRowsNum; i++) {
			...
			if (overlapRow > -1) {
				...
				viewSalesInput.model
						.setValueAt(Integer.valueOf(String.valueOf(viewSalesInput.model.getValueAt(overlapRow, 4)))
								+ Integer.valueOf(beforeList.get(i).getSellCount()), overlapRow, 4);
				// 총 가격 변경
			...
			//plusRows.addElement(beforeList.get(i).getSellCount());
...
//		*******************************************************************************************************************
	...
	private void searchNameProcess() {
		...
		for (int i = 0; i < size; i++) {
			...
			rows.addElement(results.get(i).getName());
	...
	public void setExp() {
		posdto = new PosDto();
		stockdao.expCount();
		notice.setText(ForcePos.expName.getProductName() + " 상품은 유통기한이 " + ForcePos.exp.getRealExp() + "시간 만큼 남았습니다.");
	}
